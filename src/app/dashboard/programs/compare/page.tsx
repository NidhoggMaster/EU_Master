    ...
    {result && <>
      ...
      <section className="comparison-table-wrap" aria-label="项目详细对比">
        <table className="comparison-table">
          ...
          <tbody>
            <tr><th className="comparison-row-label">学校与排名</th>{result.comparisons.map((item) => <td key={item.program.id}><strong>{item.program.universities.map((university) => university.name).join(" + ")}</strong><span>{item.program.city || item.program.universities[0]?.city || "地点未披露"}</span>{item.program.rankings.length ? item.program.rankings.map((ranking) => <small key={ranking.id}>{ranking.provider} {ranking.year} · {ranking.rank} {ranking.subject}</small>) : <small>官网未披露排名</small>}</td>)}</tr>
