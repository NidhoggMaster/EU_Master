...
- 基本材料和项目特需材料清单；首次使用一次性建立 10 项基本清单，准备勾选与文件上传相互独立
- 本地 CSV / Supabase 项目模式和显式双向复制
- 官网事务式自动刷新、字段级来源、中文概述与历史记录
- QS 2027 学校总榜与 QS 2026 项目相关学科榜，含项目口径解读和官方榜单链接
- 目录竞争力，以及申请界面的 `AI Beta` 背景适配度、证据覆盖率、准备度、版本化权重与可选概率先验
...
另有 15 个 2027 规划项目的官网基线，以及 14 所学校所在城市的生活费资料。同步命令会同时写入本地目录与 Supabase 项目目录：

```bash
pnpm catalog:expand
```

项目总数为 28。生活费优先按项目实际城市解析，再回退学校城市；因此 JADS 使用 's-Hertogenbosch，而不是 Tilburg。官网仅给最低资金或总额时会保留来源并给出保守上限，界面明确区分“官网区间”和“官网基准保守估算”。Leiden 已停止招收新生的 `ICT in Business and the Public Sector` 没有作为活动项目录入，目录改为当前后继项目 `Computer Science — Business Studies`。
...
排名基线覆盖 13 所进入 QS 2027 总榜的荷兰大学，28 个项目均继承合作学校总榜；其中原有 13 个方向另有最接近的 QS 2026 学科榜。Open University of the Netherlands 保留为“未录入 QS 总榜”。项目详情将学校总榜与项目相关学科榜分开显示。联合项目展示各合作院校的学科底盘，不把学科榜误写成项目自身排名；这些手工确认事实会锁定 `rankings` 字段，官网刷新不会静默覆盖。
...
个人背景适配分析只在申请界面显示，并带有 `AI Beta` 标识；项目目录不保存针对个人背景的判断。该分数只根据本机结构化档案、材料状态和已确认官网条件辅助比较，不代表学校审核或录取结果。
